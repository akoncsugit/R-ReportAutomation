...
## Subsetting the data
We will subset the data based on the category listed in our YAML header. In this case, using data from ``r paste0("data_channel_is_",params$category)``. We will remove non-predictors such as `url` and `timedelta` and selected our desired predictors** and `shares`.
...
`lm(shares ~  n_tokens_content*num_imgs + n_tokens_content*num_videos + 
              ...
              data = training_data)`
```{r full-lm-model-for-model-selection, eval = FALSE}
