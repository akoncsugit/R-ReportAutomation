...
correlation_matrix %>% select(shares)

shares_corr<- correlation_matrix %>% select(shares)
shares_corr

shares_strongest_corr <- sharesCorr %>%
  filter(abs(shares) > 0.025 & abs(shares) !=1) %>%
  rownames_to_column(var = "predictor") %>%
  arrange(desc(abs(shares)))
shares_strongest_corr

# Genterates scatter plots for the strongest correlations with shares
listPred <- as.list(shares_strongest_corr$predictor)


corP <- function(x) {
  var1 <- get(x, train_no_url)
  plot(var1,  train_no_url$shares)
}
