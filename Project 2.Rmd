...
subset_data <- online_new_popularity_data %>%
  ...
  select(-starts_with("data_channel_is")) %>% select(global_subjectivity, 
...
colnames(summary) <- c("NA or Infinite values")
summary %>% kable()
```
...
```{r}
IQR<- as_tibble(lapply(training_data[ , 1:11], IQR))
IQR
```
...
correlations <- as_tibble(round(cor(training_data), 3))
...
```{r}
weekend_factor <- training_data %>%
  mutate(weekend = as.factor(is_weekend)) %>%
  mutate(shares = shares/(2*sd(shares))) %>% select(weekend, shares)

g <- ggplot(weekend_factor, aes(x=shares))
g + geom_density(adjust = 0.5, alpha = 0.5, aes(fill = weekend),
                 position = "stack") 
```

```{r}
g + stat_ecdf(geom = "step", aes(color = weekend)) +
  ylab("ECDF")
